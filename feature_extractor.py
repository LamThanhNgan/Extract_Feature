...
def generate_external_dataset(header):
    db = pd.read_csv('data/URL_result_alive.csv')
    lst = list(db['url'])
    lst = list(set(lst))

    dataset = pd.read_csv('data/URL_result_alive.csv')
    i = 0 
    nb = 0
    if not os.path.isfile('dataset_result.csv') :
        with open('dataset_result.csv', 'w', newline="") as csvfile :
    ...
    for row in lst:
        ...
        if url in lst:
            ...
            # print("RRES: ", res)
            if res!=None:
                with open('dataset_result.csv', 'a', newline="") as csvfile :
                    writer = csv.writer(csvfile)
                    writer.writerow(res)
                    csvfile.close()
                state = 'Ok'
